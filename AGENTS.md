...
- **Unit tests** (in-module `#[cfg(test)]`, 32 across 8 modules): `commands.rs` (sanitize/transcode concurrency/subdir normalization/remove-files), `naming.rs` (slugify, unique-name suffixing), `db.rs` (audio update/batch), `generator.rs` (escape_hoi4, localisation UTF-8 BOM), `hoi4_version.rs` (10, version extraction), `station.rs` (find_by_name/delete), `audio.rs` (transcode args force stereo, ffprobe ID3 parsing), `validator.rs` (custom ffprobe path). All synchronous `#[test]`.
- **Integration tests** (`src-tauri/tests/`, 15 tests in 8 files): lifecycle tests (`db_test` incl. the v2→v3 migration check, `station_test` incl. subdir round-trip, `audio_repo_test`, `models_test`), output tests (`generator_test` x5, incl. custom and name-derived folders), validation (`validator_test` x3, `#[tokio::test]`), import (`import_batch_test`). Pattern: `tempfile::tempdir()` + `Db::open(tmp/...app.db)`; dummy OGG/WAV fixtures are hand-written bytes.
