...
| 任务 | 说明 |
|---|---|
| 导入完成后再建立项目引用 | 避免项目音频列表短暂出现 pending 条目 | 🔄 待做 | |
| 生成可读电台/歌曲 ID | 替代 `station_<uuid>` / `audio_<uuid>`，便于调试和与现有 Mod 风格一致 | 🔄 待做 | |
| 拖拽导入 | 提升音频库交互效率 | 🔄 待做 | |
...
`plans/2026-06-15-hoi4-compatibility-and-quality-plan.md` 的 4 个高优先级任务（9.2）与「拆分哈希/转码并发度」（9.3）已全部完成并合并至 main（`dfcb175` / `c8b5c97` / `d3f474c` / `29f7702` / `8ed7843`），`cargo test --lib` 27 项单元测试与 10 项集成测试全部通过。

下一步继续 9.3 剩余中优先级体验项：建议先做「生成可读电台/歌曲 ID」，随后「导入完成后再建立项目引用」，最后「拖拽导入」。
