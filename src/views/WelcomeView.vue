<template>
  <div class="d-flex align-center justify-center bureau-hero">
    <div class="hero-content text-center pa-8">
      <div class="hero-badge mb-6">
        <span class="text-mono text-caption">BROADCAST BUREAU</span>
      </div>
      <h1 class="text-display hero-title mb-4">HOI4 Radio Maker</h1>
      <p class="text-body hero-subtitle mb-8">
        为你的钢铁雄心 IV 战役谱写声音。
        <br />
        创建项目、导入音频、编辑电台、一键生成 Mod。
      </p>
      <v-btn
        color="primary"
        size="large"
        prepend-icon="mdi-radio-tower"
        class="hero-cta"
        @click="emitCreate"
      >
        新建广播项目
      </v-btn>
    </div>
  </div>
</template>

<script setup lang="ts">
function emitCreate() {
  window.dispatchEvent(new CustomEvent('bureau:create-project'))
}
</script>

<style scoped>
.bureau-hero {
  position: relative;
  min-height: 100%;
  height: 100%;
  width: 100%;
}

.bureau-hero::before {
  content: '';
  position: absolute;
  inset: 0;
  background: radial-gradient(circle at 50% 50%, rgba(var(--v-theme-primary), 0.08) 0%, transparent 50%);
  pointer-events: none;
}

.bureau-hero::after {
  content: '';
  position: absolute;
  inset: 0;
  background: linear-gradient(
    90deg,
    transparent 119px,
    rgba(var(--v-theme-primary), 0.015) 119px,
    rgba(var(--v-theme-primary), 0.015) 120px,
    transparent 120px
  );
  background-size: 120px 100%;
  -webkit-mask-image: linear-gradient(90deg, transparent 0%, black 8%, black 92%, transparent 100%);
  mask-image: linear-gradient(90deg, transparent 0%, black 8%, black 92%, transparent 100%);
  pointer-events: none;
}

.hero-content {
  position: relative;
  max-width: 640px;
  animation: heroFadeIn 1s ease-out;
}

@keyframes heroFadeIn {
  from {
    opacity: 0;
    transform: translateY(24px);
  }
  to {
    opacity: 1;
    transform: translateY(0);
  }
}

.hero-badge {
  display: inline-block;
  border: 1px solid rgba(var(--v-theme-primary), 0.4);
  padding: 6px 16px;
  letter-spacing: 0.2em;
  color: rgb(var(--v-theme-primary));
}

.hero-title {
  font-size: clamp(2.8rem, 6vw, 4.5rem);
  line-height: 1.05;
  color: rgb(var(--v-theme-on-background));
  text-shadow: 0 2px 24px rgba(var(--v-theme-primary), 0.2);
}

.hero-subtitle {
  font-size: 1.15rem;
  line-height: 1.7;
  color: rgb(var(--v-theme-on-surface-variant));
  max-width: 520px;
  margin: 0 auto;
}

.hero-cta {
  font-size: 1.05rem;
  padding: 0 28px;
  box-shadow: 0 0 24px rgba(var(--v-theme-primary), 0.25);
  transition: box-shadow 0.3s ease, transform 0.2s ease;
}

.hero-cta:hover {
  box-shadow: 0 0 36px rgba(var(--v-theme-primary), 0.4);
  transform: translateY(-2px);
}
</style>
